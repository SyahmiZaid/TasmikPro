    ...
    if (empty($firstname) || empty($lastname) || empty($email) || empty($password) || empty($confirm_password) || empty($gender)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $stmt->close();
                $teacher_count++;
                $teacher_id = $teacher_prefix . str_pad($teacher_count, 2, '0', STR_PAD_LEFT) . $date;

                // Generate unique halaqah ID
                $halaqah_prefix = "HLQ";
                $stmt = $conn->prepare("SELECT COUNT(*) AS halaqah_count FROM teacher");
                $stmt->execute();
                $stmt->bind_result($halaqah_count);
                $stmt->fetch();
                $stmt->close();
                $halaqah_count++;
                $halaqah_id = $halaqah_prefix . str_pad($halaqah_count, 2, '0', STR_PAD_LEFT) . $date;
                ...
                $stmt_teacher->bind_param("ssss", $teacher_id, $userid, $halaqah_id, $gender);
