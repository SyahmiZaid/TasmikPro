    ...
    if (empty($firstname) || empty($lastname) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $admin_prefix = "ADM";
                $stmt = $conn->prepare("SELECT COUNT(*) AS admin_count FROM admin");
                $stmt->execute();
                $stmt->bind_result($admin_count);
                $stmt->fetch();
                $stmt->close();
                $admin_count++;
                $admin_id = $admin_prefix . str_pad($admin_count, 2, '0', STR_PAD_LEFT) . $date;

                // Prepare SQL to insert data into admin table
                $stmt_admin = $conn->prepare("INSERT INTO admin (adminid, userid) VALUES (?, ?)");
                $stmt_admin->bind_param("ss", $admin_id, $userid);

                if ($stmt_admin->execute()) {
                    $success_message = "Registration successful. <a href='signin.php'>Login here</a>.";
                } else {
                    $error_message = "Error: " . $stmt_admin->error;
                }
